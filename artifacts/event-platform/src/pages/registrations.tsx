...
export default function Registrations() {
  ...
  return <AppShell><div className="page-in"><PageHeading eyebrow="ICGA organizer desk / Tickets" title="Know who is coming." description="Track tickets, headcount, waitlist, and door check-in for each ICGA program." />
    ...
    {selectedEvent && <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl border border-primary bg-primary p-5 text-primary-foreground"><UsersRound size={19} className="text-accent" /><p className="mt-5 font-mono text-[10px] uppercase tracking-[.16em] text-primary-foreground/60">Confirmed</p><p className="mt-1 font-display text-4xl font-bold">{confirmed}</p></div><div className="rounded-2xl border border-card-border bg-card p-5"><Ticket size={19} className="text-primary" /><p className="mt-5 font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">Checked in</p><p className="mt-1 font-display text-4xl font-bold">{checkedIn}</p></div><div className="rounded-2xl border border-card-border bg-card p-5"><CheckCircle2 size={19} className="text-primary" /><p className="mt-5 font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">Capacity used</p><p className="mt-1 font-display text-4xl font-bold">{selectedEvent.capacity ? Math.round((confirmed / selectedEvent.capacity) * 100) : 0}%</p></div><div className="rounded-2xl border border-card-border bg-card p-5"><Mail size={19} className="text-primary" /><p className="mt-5 font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">Waitlist</p><p className="mt-1 font-display text-4xl font-bold">{rows.filter((row) => row.status === 'waitlisted').length}</p></div></div>}
